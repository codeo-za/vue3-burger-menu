...
describe('Menu variant wrappers', () => {
  ...
  describe('slot forwarding', () => {
    for (const [name, Component] of Object.entries(allVariants)) {
      it(`${name} renders slot content inside the menu`, () => {
        ...
        expect(wrapper!.find('.bm-item-list .test-item')
            .exists()).toBe(true);
      });
    }
  });
  ...
  describe('Push DOM transforms', () => {
    it('applies translate3d to #page-wrap on open', async () => {
      mountVariant(Push);
      await openMenu();
      expect(pageWrap.style.transform)
          .toBe('translate3d(300px, 0px, 0px )');
      expect(pageWrap.style.transition)
          .toBe('all 0.5s ease 0s');
    });
    ...
    it('sets overflow-x hidden on body during open', async () => {
      mountVariant(Push);
      await openMenu();
      expect(document.body.style.overflowX)
          .toBe('hidden');
    });
    ...
    it('uses custom width in transform', async () => {
      ...
      await openMenu();
      expect(pageWrap.style.transform)
          .toBe('translate3d(250px, 0px, 0px )');
    });
  });
  ...
  describe('Reveal DOM transforms', () => {
    it('applies translate3d and position relative to #page-wrap on open', async () => {
      ...
      expect(pageWrap.style.transform)
          .toBe('translate3d(300px, 0px, 0px )');
      expect(pageWrap.style.position)
          .toBe('relative');
    });
  ...
  describe('ScaleDown DOM transforms', () => {
    it('applies 3D transform with Z offset to #page-wrap and perspective to #app', async () => {
      ...
      await openMenu();
      expect(pageWrap.style.transform)
          .toBe('translate3d(100px, 0px, -600px )');
      expect(pageWrap.style.transformStyle)
          .toBe('preserve-3d');
      expect(pageWrap.style.overflow)
          .toBe('hidden');
      expect(appDiv.style.perspective)
          .toBe('1500px');
      expect(appDiv.style.overflow)
          .toBe('hidden');
      expect(appDiv.style.height)
          .toBe('100%');
    });
  ...
  describe('ScaleRotate DOM transforms', () => {
    it('applies 3D transform with rotation to #page-wrap', async () => {
      ...
      await openMenu();
      expect(pageWrap.style.transform)
          .toBe('translate3d(100px, 0px, -600px ) rotateY(-20deg)');
      expect(pageWrap.style.transformStyle)
          .toBe('preserve-3d');
      expect(pageWrap.style.overflow)
          .toBe('hidden');
    ...
    it('resets all transforms, #app style, and body style on close', async () => {
      ...
      expect(pageWrap.style.transformStyle)
          .toBe('');
      expect(pageWrap.style.transformOrigin)
          .toBe('');
      expect(pageWrap.style.overflow)
          .toBe('auto');
      expect(appDiv.style.perspective)
  ...
  describe('PushRotate DOM transforms', () => {
    it('applies rotation with transform origin to #page-wrap', async () => {
      ...
      await openMenu();
      expect(pageWrap.style.transform)
          .toBe('translate3d(300px, 0px, 0px ) rotateY(-15deg)');
      expect(pageWrap.style.transformOrigin)
          .toBe('0% 50% 0px');
      expect(pageWrap.style.transformStyle)
          .toBe('preserve-3d');
  ...
  describe('Bubble animation', () => {
    it('applies borderRadius to .bm-menu on open', async () => {
      ...
      expect(bmMenu.style.borderRadius)
          .toBe('150% / 70%');
    });
    ...
    it('resets transitionTimingFunction on close', async () => {
      ...
      await closeMenu();
      const bmMenu = wrapper!.find('.bm-menu').element;
      expect(bmMenu.style.transitionTimingFunction)
          .toBe('');
    });
  });
});
